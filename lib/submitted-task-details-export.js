...
function getStatusLabel(completion) {
  if (completion?.status === "not_required") {
    return "Not required";
  }
...
async function writeTab(token, spreadsheetId, title, rows) {
  ...
  const response = await fetch(
    `${SHEETS_API_BASE}/${spreadsheetId}/values/${range}!A1?valueInputOption=USER_ENTERED`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
      ...
      // USER_ENTERED makes exported attachment URLs clickable in Sheets.
      // Escape formula-looking employee input so it remains plain text.
      body: JSON.stringify({ values: rows.map((row) => row.map(sanitizeSheetValue)) }),
    }
  );
  const data = await response.json();
  if (!response.ok) {

...
function collectAttachmentFiles(value, key = "", bucket = { all: [], photos: [], screenshots: [] }) {
  if (Array.isArray(value)) {
    const stringItems = value.filter((item) => typeof item === "string" && item.trim());
    if (stringItems.length) {
      bucket.all.push(...stringItems);
      const normalizedKey = key.toLowerCase();
      if (normalizedKey.includes("photo")) {
        bucket.photos.push(...stringItems);
      }
      if (normalizedKey.includes("screenshot")) {
        bucket.screenshots.push(...stringItems);
      }
    }
...
function buildSubmittedTaskDetailsRows(tasks, completions, users) {
...
async function rewriteSubmittedTaskDetailsSheet(rootDir, { tasks, completions, users }) {
  ...
  const rows = buildSubmittedTaskDetailsRows(tasks, completions, users);
